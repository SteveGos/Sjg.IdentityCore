...
{
    public class InviteRoleAddModel : PageModel
    {
        private readonly AccAuthContext _context;

        public InviteRoleAddModel(AccAuthContext context)
        {
            _context = context;
            ...
            int grid_Page = 1, int grid_Pagesize = 10, int grid_Buttoncount = 5)
        {
            ...
            if (addId != null)
            {
                if (!_context.AccAuthInviteRoles.Where(o => o.AccAuthInviteId == id.Value && o.AccessRoleId == addId.Value).AnyAsync().Result)
                {
                    _context.AccAuthInviteRoles.Add(new AccAuthInviteRole { AccAuthInviteId = id.Value, AccessRoleId = addId.Value });
            ...
            var curGroupRoles = _context.AccAuthInviteRoles.Where(o => o.AccAuthInviteId == id);

            var qry = from accessRole in _context.AccessRoles
                      join AccAuthInviteRole in curGroupRoles on accessRole equals AccAuthInviteRole.AccessRole into joinSet
                      from item in joinSet.DefaultIfEmpty()
                      where item.AccAuthInvite == null
                      select accessRole;
